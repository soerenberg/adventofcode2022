...
data S = S { _cave        :: S.Set Vec2  -- M.Map Vec2 Bool
           , _rockIndex   :: Int
           , _jet         :: M.Map Int Int
           , _jetCount    :: Int
           , _jetNum      :: Int
...
moveInitRock r =
  do (s1, s2, s3) <- pop3Jets
     case (s1+s2, s1+s2+s3, _kind r) of
...
fall r = do let r' = r { _cells = map (\(x,y)->(x,y-1)) $ _cells r
                       , _boundT = _boundT r - 1  -- TODO lenses?
                       , _boundB = _boundB r - 1}  -- TODO lenses?
            feas <- isFeasible r'
...
simLevel :: State S Int
simLevel =
  do done <- (>=2022) <$> (epoch <<%= (+1))
     if done
       then use level
       else do rc <- rockIndex <<%= (`mod` 5) . (+1)
...
main = do input <- ((!!0) . lines) <$> readFile "data/day17.txt"
          let r = evalState simLevel (initState input)
